...
class Application:
    ...
    def __init__(self):
        ...
        # A requisição da LLM é executada fora da thread do Pygame para a janela
        # continuar responsiva enquanto a API processa a análise.
        self.llm_executor=ThreadPoolExecutor(max_workers=1, thread_name_prefix="llm-analysis")
    ...
    def _build_controls(self):
        ...
        self.buttons["Analisar com IA"]=pygame_gui.elements.UIButton(pygame.Rect(210,570,180,35),"Analisar com IA",self.manager)
        self.route_buttons = {
    ...
    def _click(self, button):
        try:
            ...
            if button is self.route_buttons[ROUTE_GENETIC]:
                ...
                if self._llm_is_running():
                    self.runner.message="Aguarde a análise da IA terminar antes de iniciar outro processamento."
                    return
                self._close_llm_window()
                ...
                if self._llm_is_running():
                    self.runner.message="Aguarde a análise da IA terminar antes de limpar os resultados."
                    return
                self._close_llm_window()
                ...
                self._export_result()
            elif button==self.buttons["Analisar com IA"]:
                self._start_llm_analysis()
        except (ValueError, TypeError) as error:
            self.runner.fail(error)

    def _llm_is_running(self):
        return self.llm_future is not None and not self.llm_future.done()

    def _start_llm_analysis(self):
        if self._llm_is_running():
            self.runner.message="A análise com IA já está em andamento."
            return

        try:
            payload=build_analysis_payload(self.runner,self.current_scenario)
            analyzer=LlmAnalyzer()
        except Exception as error:
            message=str(error)
            self.runner.message=message
            self._show_llm_message("Configuração da IA",message)
            return

        self.buttons["Analisar com IA"].set_text("Analisando...")
        self.buttons["Analisar com IA"].disable()
        self.runner.message="Análise com IA em andamento..."
        self.llm_future=self.llm_executor.submit(analyzer.analyze,payload)

    def _poll_llm_analysis(self):
        if self.llm_future is None or not self.llm_future.done():
            return

        self.buttons["Analisar com IA"].set_text("Analisar com IA")
        self.buttons["Analisar com IA"].enable()

        try:
            analysis=self.llm_future.result()
            self.runner.message="Análise com IA concluída."
            self._show_llm_message("Análise da IA",analysis)
        except Exception as error:
            message=f"Não foi possível concluir a análise: {error}"
            self.runner.message=message
            self._show_llm_message("Falha na análise da IA",message)
        finally:
            self.llm_future=None

    def _show_llm_message(self,title,message):
        self._close_llm_window()
        safe_message=html.escape(str(message)).replace("\n","<br>")
        self.llm_window=pygame_gui.windows.UIMessageWindow(
            pygame.Rect(325,125,850,730),
            safe_message,
            self.manager,
            window_title=title,
        )

    def _close_llm_window(self):
        if self.llm_window is not None:
            try:
                self.llm_window.kill()
            except Exception:
                pass
            self.llm_window=None
